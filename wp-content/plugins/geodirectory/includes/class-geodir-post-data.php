...
class GeoDir_Post_Data {
	...
	public static function add_listing_form( $params = array() ) {
		...
		if ( $user_id && isset( $_REQUEST['pid'] ) && $_REQUEST['pid'] != '' ) {
			...
			if ( ! self::can_edit( $post_id, get_current_user_id(), $parent_id ) ) {
				echo self::output_user_notes( array( 'gd-error' => __( 'Немате дозволу да ово уређујете.', 'geodirectory' ) ) );
				return;
			}
			...
			if ( ! empty( $post_revisions ) ) {
				...
				$user_notes['has-revision'] = sprintf( __( 'Нашли смо неке несачуване измене од раније и приказујемо их испод. Ако желите да почнете поново, онда кликните  %sовде%s да уклоните поменуту ревизију.', 'geodirectory' ), "<a href='javascript:void(0)' onclick='geodir_delete_revision();'>", "</a>" );
		...
		elseif ( isset( $_REQUEST['listing_type'] ) && $_REQUEST['listing_type'] != '' ) {
			...
			if ( ! empty( $auto_drafts ) && isset( $auto_drafts[0] ) ) {
				...
				if ( $post->post_modified_gmt != '0000-00-00 00:00:00' ) {
					$user_notes['has-auto-draft'] = sprintf( __( 'Нашли смо објаву која је раније започета и приказујемо је испод. Ако желите да почнете поново, онда кликните %sовде%s да уклоните поменуту ревизију.', 'geodirectory' ), "<a href='javascript:void(0)' onclick='geodir_delete_revision();'>", "</a>" );
				}
			} else {
		...
		<form name="geodirectory-add-post" id="geodirectory-add-post"
			 ...
			 */
			$details_header = apply_filters( 'geodir_add_listing_details_header', __( 'Унесите детаље', 'geodirectory' ), $listing_type, $post, $package );
			if ( $details_header != '' ) {
			?>
			...
			<div id="geodir-add-listing-submit" class="geodir_form_row clear_both"
				...
				if ( geodir_get_option( 'post_preview' ) ) {
					...
					echo "<a href='$preview_link' target='wp-preview-" . $preview_id . "' class='geodir_button geodir_preview_button'>" . __( 'Преглед', 'geodirectory' ) . " <i class=\"fas fa-external-link-alt\" aria-hidden=\"true\"></i></a>";
				}
				?>
	...
	public static function delete_revision( $post_data ) {
		...
		if ( ! self::owner_check( $post_data['ID'], get_current_user_id() ) ) {
			return new WP_Error( 'gd-not-owner', __( "Ово није ваша објава", "geodirectory" ) );
		}

		$result = wp_delete_post( $post_data['ID'], true );
		if ( $result == false ) {
			return new WP_Error( 'gd-delete-failed', __( "Брисање није успело.", "geodirectory" ) );
		} else {
			return true;
		}
	}
	...
	public static function auto_save_post( $post_data, $doing_autosave = true ) {
		...
		if ( ! self::can_edit( $post_id, get_current_user_id(), $parent_id ) ) {
			return new WP_Error( 'save_post', __( "Немате овлашћења за ово.", "geodirectory" ) );
		}
	...
	public static function check_logged_out_author( $post_data ) {
		if ( ! get_current_user_id()
			...
			if ( strlen( $user_name ) < 3 ) {
				$error = new WP_Error( 'geodir_invalid_username', __( 'Корисничко име мора имати бар 3 карактера.', 'geodirectory' ) );
			} elseif ( ! is_email( $user_email ) ) {
				$error = new WP_Error( 'geodir_invalid_user_email', __( 'Неправилна email адреса.', 'geodirectory' ) );
			} else {
				...
				if ( $user_id_from_username && $user_id_from_email && $user_id_from_username == $user_id_from_email ) { // user already exists
					...
					if ( is_wp_error( $user_id ) ) {
						...
						$error = new WP_Error( 'geodir_register_new_user', __( 'Нешто није у реду! Није успело регистровање новог корисника.', 'geodirectory' ) );
					}
				}
			}
	...
	public static function ajax_save_post_message( $post_data ) {
		...
		if ( isset( $post_data['post_parent'] ) && $post_data['post_parent'] ) {
			...
			if ( $post_data['post_status'] == 'publish' ) {
				...
				$message = sprintf( __( 'Измена примљена, промене су сада видљиве %sовде%s.', 'geodirectory' ), "<a href='$link' >", "</a>" );
				...
				$message = __( 'Измена примљена, могуће је да промене морају бити прегледане пре објављивања.', 'geodirectory' );
			}
			...
			if ( $post_data['post_status'] == 'publish' ) {
				$link    = get_permalink( $post_data['ID'] );
				$message = sprintf( __( 'Објава примљена, подаци се сада могу видети  %sовде%s.', 'geodirectory' ), "<a href='$link' >", "</a>" );
			} // post needs review
			else {
				$post         = new stdClass();
				$post->ID     = $post_data['ID'];
				$preview_link = self::get_preview_link( $post );
				$message      = sprintf( __( 'Објава примљена, могуће је да промене морају бити прегледане пре објављивања, можете их видети %sовде%s.', 'geodirectory' ), "<a href='$preview_link' >", "</a>" );
			}
		}
...
######################## functions to show preview to logged out user ###########################
	...
	public static function add_listing_mandatory_note( $listing_type = '', $post = array(), $package = array() ) {
		...
		&nbsp;<?php echo __( 'Означава поље у коме је обавезан унос', 'geodirectory' ); ?></p><?php
	}
